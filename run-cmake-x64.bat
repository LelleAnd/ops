...
@IF NOT DEFINED OPS_TINYXML2_DBG_DIR goto :TINYXML2_MISSING
@IF NOT DEFINED OPS_TINYXML2_OPT_DIR goto :TINYXML2_MISSING
...
@set XML_OPTIONS_DBG=-DTINYXML2_INSTALL_DIR=%OPS_TINYXML2_DBG_DIR%
@set XML_OPTIONS_REL=-DTINYXML2_INSTALL_DIR=%OPS_TINYXML2_OPT_DIR%
...
cmake -DCMAKE_BUILD_TYPE=Bootstrap -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% -A x64 %~dp0
...
@rem cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% -DOPS_BUILD_EXAMPLES=%OPS_BUILD_EXAMPLES% -DOPS_BUILD_UNITTESTS=%OPS_BUILD_UNITTESTS% -A x64 %~dp0
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% %MISC_OPTIONS% %XML_OPTIONS_DBG% -A x64 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED
@rem cmake --build . --target ALL_BUILD --config Debug
msbuild -m ALL_BUILD.vcxproj -p:Configuration=Debug
@IF ERRORLEVEL 1 goto :BUILD_FAILED
...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% %XML_OPTIONS_REL% -A x64 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED

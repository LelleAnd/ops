...
@set LANG_OPTIONS=-DOPS_BUILD_CPP=%OPS_BUILD_CPP% -DOPS_BUILD_CSHARP=%OPS_BUILD_CSHARP% -DOPS_BUILD_PYTHON=%OPS_BUILD_PYTHON%
...
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% %MISC_OPTIONS% -A x64 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED
@rem cmake --build . --target ALL_BUILD --config Debug
msbuild -m ALL_BUILD.vcxproj -p:Configuration=Debug
@IF ERRORLEVEL 1 goto :BUILD_FAILED
...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% -A x64 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED

...
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% %MISC_OPTIONS% -A Win32 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED
@rem cmake --build . --target ALL_BUILD --config Debug
msbuild -m ALL_BUILD.vcxproj -p:Configuration=Debug
@IF ERRORLEVEL 1 goto :BUILD_FAILED
...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=%OPS_INSTALL_PREFIX% %LANG_OPTIONS% -A Win32 %~dp0
@IF ERRORLEVEL 1 goto :BUILD_FAILED
